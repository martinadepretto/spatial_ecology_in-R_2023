...
\begin{frame}
 ...
 \begin{itemize} %create a list with the function itemize
...
\begin{frame}
 \frametitle{Itemizing}
 \begin{itemize} %create a numerical list
...
\begin{frame}
 \frametitle{Itemizing}
 \begin{itemize} %create a list where the n sentences appear gradually in order, creating n different slides
  \item<1-> Remote sensing is a powerful tool
  \item<2-> It can be used for diversity estimate
  \item<3-> It can be used in ecological informatics
 \end{itemize}
...
\begin{equation}
K_{\alpha}=\frac{1}{\left( \sum_{i=1}^{N} p_i \times p_i^{\alpha-1} \right)^{\frac{1}{\alpha-1}}} % \left( means open bracket      % \right) means close brackets
\end{equation}
\end{frame}
...
\begin{frame}{The code!}
    ...
    \begin{itemize}
        \item x = input file...
        \item $\alpha$ = blablabla % way to write a symbol:  $\symbol$
    \end{itemize}
\end{frame}
